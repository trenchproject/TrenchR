...
#' @param A_d total surface area exposed to diffuse and albedo flux
...
#' @param S_d sum of the diffuse and albedo flux densities (W m^-2)
#' @param rho_body vapor density of the mussel body (kg m^-3)
#' @param hm coefficient of mass transfer (m/s)
...
Tb_mussel = function(L, H, T_a, T_g, u, p, psi, A_d, A_evap, S, S_d, rho_body, hm, cl, group = "solitary"){
  ...
  #Short-wave solar flux (coefficient)
  ...
  if (group == "aggregated") {        # derived from the relationship between Nusselt number and Reynolds number
    ...
    hc = 0.63 * Ka / L * (u * L / v)^0.47
  }
  
  
  # Evaporation
  # calculating water vapor concentration
  # From pV = nRT, n/V = p/RT (moles/L) where p is atmospheric pressure (atm), 
  ...
  T_b = (k1 * (A_sol * S + A_d * Sd) + k2 * A_radSky * k3 * T_a^4 + k4 * A_radGround * T_g^4 + k5 * A_cond * T_g + 
