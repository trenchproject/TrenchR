...
## Soil temperature
We implemented a microclimate model with air temperature and radiation as input using finite-difference methods to solve heat balance equations describing soil temperatures at the surface and specified depths. Our approach follows Porter et al. [1973] and is similar to that described for the NicheMapR R package [Kearney and Porter 2017]. The change is soil temperature ($T_s$) through time $t$ and depth $z$ can be approximated with a differential equation
...
Solving for $T_i^{p+1}$ produces as expression for solving soilt emperature at depth $i$ at any time:
$$T_i^{p+1} = a \frac{dt}{dz^2}(T_{i+dz}^p+T_{i-dz}^p)+(1-2a \frac{dt}{dz^2})T_i^p$$

Solving the equation requires two boundary conditions: 1) the deep soil temperature, and 2) the soil surface temperature. We assume that the soil temperature at 60cm is constant diurnally. The soil surface temperature is estimated using an energy balance equating the energy conducted to the soil surface the the net heat transfer to the surface by solar radiation, infrared radiation, and conduction:
$$Qnet_s = Qabs_s + Qemit_s + Qconv_s,$$
where the solar radiation absorbed by the soil surface is the product of incoming radiation $S$ and the proportion radiation absorbed $\alpha_s$, $Qabs_s= \alpha_s S$. The net thermal radiation to the soil surface is determined by balance of radiation emitted by the soil surface $\sigma T_s^4$ and sky radiation absorbed by the soil surface $\sigma T_{sky}^4$: $Qemit_s= \sigma (T_{sky}^4 - T_s^4)$. Convection between the soil surface and air is estimated as a function of the temperature difference $Qconv_s= h_s (T_-T_s)$, where $h_s$ is the surface heat transfer coefficient. Consideration of temperature profiles and surface roughness yields a expression during neutral conditions [Sellers 1965]: 
$$h_s= \frac{\rho_s c_s k^2 V_r}{ln(z_r/z_o+1)^2},$$
where $V$ is wind speed at reference height $z_r$ and $z_0$ is roughness length. 

GRIGS: EQ 10 to 13

#soil_temperature_noint CALLS soil_temperature CALLS soil_temp_overall_function CALLS soil_temp_integrand
soil_temperature_noint: sets up ODE solution 
soil_temperature: core soil temperature function 
...
soil_temp_integrand: #Integrand for relationship between air temperature and height [SEE EQ. 12 BECKMAN 1973]
...
# Estimating soil specific heat and conductivity
