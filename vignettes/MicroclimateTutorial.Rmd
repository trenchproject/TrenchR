...
We additionally provide a function (`monthly_solar_radiation()`) implementing an algorithm to estimate average monthly solar radiation ($W m^{-2} day^{-1}$) using basic topographic and climatic information as input.
...
Temperatures and wind speeds, standardly collected at a height of 2 m, can differ dramatically from conditions near the ground where most organisms live. 
...
t.seq <- lapply(z.seq, 
                ...
                T_s = 27)
plot(t.seq, 
     z.seq, type = "l", xlab = "Temperature (°C)", ylab = "z (height above ground in m)")
...
legend(x      = "topleft", 
       title  = "function", 
       legend = c("_sine", "_sineexp", "_sinesqrt"),
       lty    = c("solid", "dotted", "dashed"))
```

We adapt equations from Tham et al. [-@tham2010estimation] to estimate hourly solar radiation ($W m^{-2} hr^{-1}$) as a function of daily global solar radiation (in $W m^{-2} d^{-1}$). 
