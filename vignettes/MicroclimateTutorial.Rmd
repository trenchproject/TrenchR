...
where J is calendar date, $\phi$ is latitude (radians), h is hour, and $h_0$ is the time of solar noon. We illustrate how zenith angles (degrees) change over the day throughout the year in Seattle, WA, USA:
```{r, fig.height=4, fig.width=6}
zen= unlist(lapply(1:24, FUN=zenith_angle, doy=200, lat=47.61, lon=-122.33))
plot(1:24, zen, type="l", ylab="zenith angle (degrees)", xlab="hour")
...
We also examine how zenith angles (degrees) at noon change over the course of the year for latitudes along the longitude of Seattle, WA USA:
```{r, fig.height=4, fig.width=6}
zen= unlist(lapply(1:365, FUN=zenith_angle, hour=12, lat=60, lon=-122.33))
plot(1:365, zen, type="l", ylim=range(0,90), ylab="zenith angle (degrees)", xlab="day of year")
...
plot(5:18, az, type="l", xlab="hour", ylab="azimuth angle (degrees)")
...
plot(1:365, az, type="l", xlab="day of year", ylab="azimuth angle (degrees)", ylim=c(0,120))
...
We examine how daylength varies over the year for sites at several latitudes.
```{r, fig.height=4, fig.width=6}
plot(1:365, daylength(lat=10, doy=1:365), type="l", ylim=c(8,22), xlab="day of year", ylab="daylength (hour)")
points(1:365, daylength(lat=35, doy=1:365), type="l", lty="dashed")
points(1:365, daylength(lat=60, doy=1:365), type="l", lty="dotted")
legend("topright", title="latitude (degrees)", legend=c(10, 35, 60), lty=c("solid","dashed","dotted") )
```
...
plot(1:24, rd[1,], type="l", xlab = "hour", ylab = "radiation (W/m^2)", ylim=c(0,800))
...
legend("topright", title="zenith angle (degrees)", legend=c("direct", "diffuse", "reflected"), lty=c("solid","dotted","dashed") )
```
...
r.seq=unlist(r.seq)
plot(seq(1,365,31), r.seq, type = "l", xlab = "day of year", ylab = "radiation (W/m2)")
```
...
plot(seq(0,1,0.1), r.seq, type = "l", xlab = "clearness index, k_t", ylab = "radiation (W/m2)", ylim=c(0,1))
...
Temperatures and windspeeds, standardly collected at a height of 2m, can differ dramatically from conditions near the ground where most organisms live. Turbulent air flow influenced by the ground surface roughness sets up temperature and windspeed profiles.  These profiles can be estimated to scale temperatures and widspeeds to organism height. We scale across height by estimating a wind speed profile using the relationship:
...
legend("bottomright", title="windspeed at 2m", legend=c(0.25, 5, 1), lty=c("dotted","dashed","solid") )
```
...
We additionally provide functions for estimating wind speed and temperature profiles for the case of free convection using the relationships in NicheMapR as described by Kearney and Porter [-@kearney2017nichemapr].  The function `wind_speed_profile()` estimates wind speed at height $z$ as $u_z=2.5u^*\ln(z/z_0+1)$ where the friction velocity $u^*$ is estimated as $u^*=\frac{0.41u_r}{\ln(z/z_0+1)}$ and 0.41 is the von Karman constant.

```{r, fig.height=4, fig.width=6}
zs= seq(0,2,0.1) 
us= wind_speed_profile(u_r=1, zr=2, z0=0.2, z=zs)
plot(us,zs, type="l", xlab = "wind speed, u (m/s)", ylab = "height, z (m)")
points(wind_speed_profile(u_r=0.5, zr=2, z0=0.2, z=zs), zs, type="l", lty="dashed")
points(wind_speed_profile(u_r=0.25, zr=2, z0=0.2, z=zs), zs, type="l", lty="dotted")
