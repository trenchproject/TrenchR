...
## Soil temperature at nodes between surface and 60cm deep 
...
$$Q_net=Qabs_s= + Qemit_s + Qconv_s + Qcond_s,$$
...
##GRIGS SURFACE BOUNDARY CONDITION
$$Q_{net}= Q_{sol_s} + Q_{emit_s} + Q_{conv_s} +Q_{cond_s}$$
where
$$Q_{net}= \frac{\rho_s c_s dz}{2 dt} (T_0^{p+1}-T_0^p)$$
$Q_{sol_s}= \alpha_s S$
$Q_{emit_s}= \epsilon_s \sigma [(T^p_{sky})^4-(T^p_0)^4]$
$Q_{conv_s}= h_s(T^p-T^p_s)$
$Q_{cond_s}=k_s \frac{\delta T_s}{\delta z}$, at z=0. Approximating $q_cond$ by finite difference yields $Q_{cond_s}=\frac{k_s}{dz}(T^p_{dz}-T^p_s), at z=0$

where h_s as above
$T_sky=0.552 T^{1.5}$

letting $a=\frac{2dt}{\rho_s \c_s dz}
solving for $T_0^{p+1}$ yields
