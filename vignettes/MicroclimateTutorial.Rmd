...
We illustrate the implementation of these calculations in TrenchR. The day of year and solar declination angle can be calculated as follows:
```{r}
...
where J is calendar date, $\phi$ is latitude (radians), h is hour, and $h_0$ is the time of solar noon. We illustrate how zenith angles (degrees) changes over the day for days throughout the year in Seattle, WA, USA:
```{r}
#across hours
...
TrenchR includes a function to estimate azimuth angle as follows:
```{r}
...
```{r}
#Radiation as a function of hour
zen= unlist(lapply(1:24, FUN=zenith_angle, doy=200, lat=47.61, lon=-122.33))
zen= zen*2*pi/360
...
#Radiation as a function of zenith angle
...
plot(seq(0,1,0.1), r.seq, type = "l", xlab = "k_t", ylab = "radiation (W/m2)", ylim=c(0,1))
...
Temperatures and windspeeds collected at ~2m can differ dramatically from conditions near the ground where most organisms live. Turbulent air flow influenced by the ground surface roughness sets up temperature and windspeed profiles.  These profiles can be estimated to scale temperatures and widspeeds to organism height. We scale across height by estimating a wind speed profile using the relationship:
...
legend("bottomright", title="windspeed at reference height of 2m", legend=c(0.25, 5, 1), lty=c("dotted","dashed","solid") )
```
...
t.seq <- air_temp_profile_neutral(T_r=20, zr=1, z0=0.5, z=z.seq, T_s=25)
points(t.seq, z.seq, type = "l", lty="dotted")
t.seq <- air_temp_profile_neutral(T_r=20, zr=1, z0=0.8, z=z.seq, T_s=25)
points(t.seq, z.seq, type = "l", lty="dashed")

legend("topright", title="surface roughness", legend=c(0.2, 0.5, 0.8), lty=c("solid","dotted","dashed") )
```
...
plot(t.seq, z.seq, type = "l", xlab = "Temperature (C)", ylab = "z (height above ground in m)")
t.seq <- air_temp_profile(T_r = 20, u_r = 0.5, zr = 2, z0 = 0.5, z = z.seq, T_s = 25)
points(t.seq, z.seq, type = "l", lty="dotted")
t.seq <- air_temp_profile(T_r = 20, u_r = 0.5, zr = 2, z0 = 0.8, z = z.seq, T_s = 25)
points(t.seq, z.seq, type = "l", lty="dashed")

legend("topright", title="surface roughness", legend=c(0.2, 0.5, 0.8), lty=c("solid","dotted","dashed") )
```

Trench R also adapts functions from NicheMapR [Kearney and Porter 2016] to refine wind speed and air temperature profiles by using multiple segments:
```{r}
z.seq <- seq(0, 2, by = 0.1)
...
Soil temperature can be estimated in TrenchR as follows:
```{r}
library(deSolve)
...
T_soil= soil_temperature(z_r.intervals=12,z_r=1.5, T_a=temp_vector, u=wind_speed_vector, Tsoil0= 20, z0=0.02, SSA=0.7, TimeIn=time_vector, H= solrad_vector, water_content=0.2, air_pressure=85, rho_so=1620, shade=FALSE)
