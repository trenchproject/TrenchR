...
```{r}
degree_days(Tmin=7, Tmax=14, LDT=12, UDT=33, method="single.sine")
degree_days(Tmin=7, Tmax=14, LDT=12, UDT=33, method="single.triangulation")
...
hrs=1:24
Thr= sapply(hrs, FUN=diurnal_temp_variation_sinesqrt, tr=Tr, ts=Ts, Tmx=Tmx, Tmn=Tmn,Tmn_p=Tmn) 
Thr1= sapply(hrs, FUN=diurnal_temp_variation_sine, Tmx=Tmx, Tmn=Tmn)
Thr2= sapply(hrs, FUN=diurnal_temp_variation_sineexp, Tmx=Tmx, Tmn=Tmn, tr=Tr, ts=Ts, alpha=2.59, beta= 1.55, gamma=2.2)
...
```{r}
DD1= degree_days(Tmin=Tmn, Tmax=Tmx, LDT=12, UDT=33, method="single.sine")
DD2= degree_days(Tmin=Tmn, Tmax=Tmx, LDT=12, UDT=33, method="single.triangulation")
```
...
# z_r.intervals is the number of intervals in the soil profile to calculate 
# z_r is reference height in m
# Tsoil0 is the initial soil temperature in degrees C 
# z0 is surface roughness in m 
# SSA is the solar absorbtivity of soil surface as a fraction
# TimeIn is a vector of time periods for the model
# H is solar radiation in W m^-2
# water_content is percent water content (%)
# p is air pressure in kPa
# rho_so= 1620 particle density of soil
# shade is whether or not soil temperature should be calculated in the shade, TRUE or FALSE

#require in package?
library(zoo)
library(deSolve)

#need to update conductivity and specific_heat function names

Tsoil=soil_temperature(z.intervals=12,z=1.5, Tair=Thr, u_z=Vs, Tsoil0= 20, z0=z0, SSA=0.7, TimeIn=hrs, solrad= Qhr_tot, water_content=0.2, air_pressure=airpressures, rho_so=1620, shade=FALSE)
