...
Calculations of operative environmental temperatures are based on balancing energy input and output to organisms.  The following accounting of heat gains and losses at the surface of the organism can be used to estimate $T_e$ [@gates2012biophysical]:
$$Q_{net} = Q_{abs} - Q_{emit} - Q_{conv} - Q_{cond} - Q_{met} - Q_{evap},$$
where $Q_{net}$ is the net energy exchange with the environment (W), $Q_{abs}$ is the solar and thermal radiation absorbed (W), $Q_{emit}$ is the thermal radiation emitted (W), $Q_{conv}$ is energy exchange due to convection (W), $Q_{cond}$ is energy exchange due to conduction (W),
$Q_{met}$ is the energy emitted due to metabolism (W), and $Q_{evap}$ is the energy emitted due to evaporative water loss (W).
...
Both variables take as parameters the animal's surface area, $A (m^2)$, and the proportion of the surface area in contact with the substrate, $\mbox{proportion}$. 
