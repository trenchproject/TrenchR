...
legend(x      = "topleft", 
       title  = "parameters", 
       legend = c("psa_dir= 0.4, T_g = 20", "psa_dir= 0.2, T_g = 20", "psa_dir= 0.4, T_g = 10", "psa_dir= 0.2, T_g = 10"), 
       lty    = c("solid", "dashed", "dotted", "dotdash"))
...
plot(x    = seq(0, 3, 0.25), 
     ...
     ylab = expression("heat transfer coefficient," ~ H[L] ~ (W ~ m^{-2} ~ C^{-1})))
...
plot(x    = 20:40, 
     y    = Qconduction_animal(T_g        = 20:40,
                               T_b        = 30,
                               d          = 10^-6,
                               K          = 0.3,
                               A          = 10^-3,
...
plot(x    = 20:40, 
     y    = Qconduction_substrate(T_g        = 20:40,
                                  T_b        = 30,
                                  D          = 0.01,
                                  K_g        = 0.3,
                                  A          = 10^-2,
...
# kPa
  ...
  vp <- saturation_vapor_pressure(20:40)
...
# kPa
  ...
  vp <- saturation_vapor_pressure(20:40-10)
...
TrenchR primarily includes models for estimating the body temperatures (generally the operative temperatures, $T_e$) of organisms that have reached an equilibrium with their environment ("steady-state", no heat retention). The following function uses the relationships above to solve the energy balance for $Tb (C)$.
...
where $A_c$ is the convective heat transfer surface area and $T_a$ is the air temperature ($C$). We assume $A_c=A_t= A_{s,ttl}$. The total convective heat transfer coefficient, $h_T Wm^{-2}K^{-1}$, is calculated as the boundary layer conductance $h_c Wm^{-2}C^{-1}$ and the fur layer conduction in series:
...
```{r, fig.height = 4, fig.width = 4}
par(mar = c(5,  5, 3, 2))
blr.seq= lapply(20:40, FUN=boundary_layer_resistance, e_s= 2.5, e_a = 2.3, elev = 500, D = 0.007, u = 2)
plot(x    = 20:40, 
     y    = blr.seq, 
     type = "l", 
     xlab = "air temperature (C)", 
...
for (hr in 1:24) {
  Qconv[hr] <- Qconvection(T_a        = Ta[hr],
                           T_b        = Te[hr],
                           H          = H_L,
                           A          = A, 
                           proportion = 0.67, 
...
for (hr in 1:24) {
  Qcond[hr] <- Qconduction_animal(T_g        = Ts[hr],
                                  T_b        = Te[hr],
                                  d          = svl / 3,
                                  K          = 0.5,
                                  A          = A, 
