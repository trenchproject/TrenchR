...
We built a biophysical model `Tb_limpet()` from the studies done by Denny and Harley (2006) to predict the body temperature of limpets based on body size (length and height), environmental conditions and the position of the limpet. The shape of the limpet is treated as a cone of radius $r$ and height $H$ for simplification.
...
The rate at which solar radiation provides heat is $W_{sw} = A_p\alpha_{sw}I_{sw}$, where $A_p$ is the area of the limpet’s shell (m^2^) projected in the direction of the sunlight, $\alpha_{sw}$ is the short-wave absorptivity of the shell, and $I_{sw}$ is the solar irradiance (W m^-2^). Under the assumption that limpet has a shape of a cone, when $tan(\psi) > r / h$, 
$$A_p = \pi r^2  cos(\psi)$$
and otherwise,
$$A_p = \pi r^2  cos(\psi) + hrsin(\psi) - \frac{\pi r^2}{2}cos(\psi)$$
where $\psi$ is the solar zenith angle (Pennell and Deignan 1989). $\alpha_{sw}$ was emperically acquired to be 0.68 for limpets (Denny and Harley 2006).
