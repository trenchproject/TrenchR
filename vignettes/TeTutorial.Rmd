...
$$Qcond= A*proportion*K*(T_b-T_g)/d, $$
...
TrenchR primarily includes models for calculating the operative temperatures of organisms that have reached an equilibrium with their environment ("steady-state", no heat retention). The following function uses the relationships above to solve the energy balance for $Te$.
```{r, fig.height=4, fig.width=6}
...
We adapted a biophysical model (`Tb_lizard()`) to calculate operative environmental temperature, $T_e$, for lizards from Buckley [-@buckley2007linking]. The function follows the modeling approach contained Campbell and Norman [-@campbell2012introduction]. 
...
We calculate the surface area by approximating the body of a female grasshopper as a rotational ellipsoid [@samietz2005altitudinal].  The major axis is equal to the grasshopper’s length.  We calculate the semi-minor axis (half of the grasshopper’s width) as $a=(0.365+0.241*1000 L)/1000$ using a regression from Lactin and Johnson [-@lactin1998environmental]. If $e=\sqrt{1-a^2/c^2}$, surface area can be calculated as follows:
...
$$Q_c= h_T A_c (T_b-T_a). $$
We calculate $h_{cs}$ from the convective heat transfer coefficient as $h_{cs}=h_c(-0.007z/L+1.71)$ where $z$=0.001m is the height above the ground. We use an empirically-derived relationship for grasshoppers to estimate the heat transfer coefficient, $h_c (Wm^{-2}*°C^{-1})$ [@lactin1998environmental]: $h_c = Nu K_f/L$, where the thermal conductivity of fluid, $K_f(W m^{-1}K^{-1})=0.024+0.00007(T_a+273)$.
...
We adapted a biophysical model for intertidal mussels which was developed by Helmuth [-@helmuth1998intertidal] to predict mussel body temperature ($T_b$) based on environmental conditions, body size (height and length) and whether the mussel is solitary or aggregated.
In general, amount of heat stored within a mussel body ($Q_{stored}$) can be described as
$$Q_{stored} = Q_{sol} \pm Q_{rad,sky} \pm Q_{rad, ground} \pm Q_{cond} \pm Q_{conv} - Q_{evap} \tag{1}$$
where $Q_{sol}$ is the short-wave solar radiation to the mussel, $Q_{rad,sky}$, $Q_{rad,ground}$ are the infrared radiation between the mussel and the sky, and between the mussel and the surroundings, $Q_{conv}$ is the convective heat exchange, $Q_{cond}$ is the conduction to and from the ground, and $Q_{evap}$ is the evaporative cooling from water loss [@helmuth1998intertidal].
Since $Q_{stored}$ is a product of mass ($m$), specific heat ($c$), and body temperature ($T_b$), the rate of change of stored heat can be expressed as $Q_{stored} = d(mcT_b)/dt$.

We calculate $Q_{sol}$ from $Q_{sol} = \frac{\alpha}{sin\theta} A_{sol}S$ where $\alpha$ is the solar absorptivity, $\theta$ is the solar elevation angle (radian), $A_{sol}$ is the mussel area in direction of the sun, $S$ is the direct solar flux density on the area [@helmuth1998intertidal]. 
...
In terms of convective heat flux ($Q_{conv}$), it can be calculated from $Q_{conv}=h_cA_{conv}(T_b-T_a)$ where $h_c$ is the coefficient for forced convection and $A_{conv}$ is total surface area exposed to convective heat loss [@helmuth1998intertidal]. Using the three empirical formulas on Nusselt number and Reynolds number $Nu = \frac{h_c L}{K_a}$, $Re = \frac{uL}{v}$ and $Nu = aRe^b$, the value of $h_c$ can be calculated. $L$ is the length of the mussel, $K_a$ is the conductivity of air $K_a = 0.00501 + 7.2 \times10^{–5}T_a (W·m^{–1}·K^{–1})$, $u$ is the wind speed (m/s), $v$ is the kinematic viscosity of air $v = -1.25 \times 10^{-5} + 9.2 \times 10^{-8} T_a$ [@denny2006hot]. $A_{conv}$ can be considered to be equal to the total surface area. $a$ and $b$ in the formula to obtain $Nu$ vary depending on the position and the state of the mussels, and they are empirically acquired. For solitary mussels, $a = 0.38$ and $b = 0.51$ if the anterior or posterior end are facing upwind, and $a = 0.63$ and $b = 0.47$ if the valve is facing upwind. The values were measured to be $a = 0.67$, $b = 0.42$ for aggregated mussels [@helmuth1998intertidal].

Lastly, $Q_{evap} = \lambda \dot{m}$ where $\lambda$ is the latent heat of vaporization of water (J/kg) and $\dot{m}$ is the rate of water loss [@helmuth1998intertidal]. For the purposes of this package, we set the value to zero to illustrate conditions when the mussel does not gape or otherwise cannot evaporatively cool.

For the usability, `Tb_mussel` function examines the steady-state (equilibrium) body temperature of an organism exposed over long periods of time to constant environmental conditions.
Inserting the values in equation (1) and solving for $T_b$ gives us the following equation.
$$T_{b,steady} = (k_1 A_{sol} S + k_2 A_{rad,sky} k_3 T_a^4 + k_4 A_{rad,ground} T_g^4 + k_5 A_{cond} T_g + h_c A_{conv} T_a) / (k_2 A_{rad,sky} T_a^3 + k_4 A_{rad, ground} T_g^3 + k_5 A_{cond} + h_c A_{conv})$$
Here, $k_1$ to $k_5$ are physical constants and measured coefficients.

We provide the operative temperature model as follows
<!-- {r, fig.height=4, fig.width=6} -->

<!-- t.seq <- lapply(20:40, FUN = Tb_mussel, L = 0.15, H = 0.05, T_g = 30, u = 0.2, psi = 30, S = 800, c = 0, group = TRUE) -->
<!-- plot(20:40, t.seq, type = "l", xlab = "ambient temperature (°C)", ylab = "body temperature (°C)", ylim = c(25, 40)) -->
<!-- t.seq <- lapply(20:40, FUN = Tb_mussel, L = 0.15, H = 0.05, T_g = 30, u = 1, psi = 30, S = 800, c = 0, group = TRUE) -->
<!-- points(20:40, t.seq, type = "l", lty = "dashed") -->
<!-- t.seq <- lapply(20:40, FUN = Tb_mussel, L = 0.15, H = 0.05, T_g = 30, u = 5, psi = 30, S = 800, c = 0, group = TRUE) -->
<!-- points(20:40, t.seq, type = "l", lty = "dotted") -->
<!-- abline(a=0,b=1, col="gray") -->

<!-- legend("bottomright",title= expression("Wind speed, m/s"), legend=c(0.2,1,5), lty=c("solid", "dashed","dotted") ) -->
...
We built a biophysical model `Tb_limpet()` from the studies done by Denny and Harley [-@denny2006hot] to predict the body temperature of limpets based on body size (length and height), environmental conditions and the position of the limpet. In this model, the shape of the limpet is approximated to a cone of radius $r$ and height $H$.

The rate at which energy is stored within the organisms ($W_{stored}$) are influenced by several heat exchanging processes. The processes include the rate of heat absorbed from short-wave radiation ($W_{sw}$), net heat transferred by long-wave radiation ($W_{lw}$), rate of heat transferred by conduction and convection ($W_{cd}$ and $W_{cv}$). We neglected the heat gain by metabolism due to the nature of the limpets' low metabolic rate, and evaporative heat loss assuming that the limpet has its shell firmly clamped against the substratum, and thereby preventing evaporation.
...
Under the assumption that limpet has a shape of a cone, when $tan(\psi) > r / h$, 
$$A_p = \pi r^2  cos(\psi)$$
and otherwise,
...
$A_{cv}$ is the area of the shell in contact with the air, which is approximated to the lateral area of the limpet ($A_l$). $h_c$ is the convective heat transfer coefficient (W m^-2^ K^-1^), and it can be expressed from Nusselt number $Nu = \frac{h_c R}{K_a}$ where $R$ is the diameter and $K_a$ is the conductivity of air.
...
where $A_{cd}$ is the area of contact between the organism's foot and the rock, and $K_r$ is the conductivity of the rock (W m^-1^ K^-1^). $T_r$ is the temperature of the rock as a function of distance $z$ (m) from the surface.

--Not sure how we would define $W_{cd}$ in the function yet.--
...
We provide the operative temperature model as follows
<!-- {r, fig.height=4, fig.width=6} -->
<!-- t.seq <- lapply(20:40, FUN = Tb_limpet, L = 0.0176, H = 0.0122, T_r = 30, I = 900, u = 1, psi = 30, c = 0, position = "anterior") -->
<!-- plot(20:40, t.seq, type = "l", xlab = "ambient temperature (°C)", ylab = "body temperature (°C)", ylim = c(25, 45)) -->
<!-- t.seq <- lapply(20:40, FUN = Tb_limpet, L = 0.0176, H = 0.0122, T_r = 30, I = 1300, u = 1, psi = 30, c = 0, position = "anterior") -->
<!-- points(20:40, t.seq, type = "l", lty = "dashed") -->
<!-- t.seq <- lapply(20:40, FUN = Tb_limpet, L = 0.0176, H = 0.0122, T_r = 30, I = 1600, u = 1, psi = 30, c = 0, position = "anterior") -->
<!-- points(20:40, t.seq, type = "l", lty = "dotted") -->

<!-- legend("bottomright",title= expression("Solar irradiance, W/m^2"), legend=c(500, 800, 1200), lty=c("solid", "dashed","dotted") ) -->
...
Suppose we want to estimate the hourly body temperature (operative environmental temperature, $T_e$) of a *Sceloporus* lizard on June 1, 2018 in Santa Fe, New Mexico, USA (35.69, -105.944, elevation: 2121m). Let's assume the lizard is in an unshaded location where the daily air temperature varies from a minimum of 10°C to a maximum of 25°C, the soil surface temperature varies from a minimum of 15C to a maximum of 30°C, and the windspeed is 0.5 m/s. We assume that atmospheric transmissivity $\tau=0.7$ and albedo $\rho=0.6$. 
