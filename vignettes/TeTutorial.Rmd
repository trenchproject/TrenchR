...
plot(seq(100,1000,100), Qradiation_absorbed(a=0.9, A=1, psa_dir=0.4, psa_ref=0.4, S_dir=seq(100,1000,100), S_dif=200, a_s=0.5), type="l", xlab = "direct solar radiation (W/m^2)", ylab = "solar radiation absorbed (W)")
...
```{r, fig.height=4, fig.width=8}
plot(seq(0,5,0.2), Reynolds_number(V=seq(0,5,0.2), D=0.001, nu=1), type="l", xlab = "wind speed (m/s)", ylab = "Reynolds number")

points(seq(0,5,0.2), Reynolds_number(V=seq(0,5,0.2), D=0.001, nu=1.5), type="l", lty="dashed")
points(seq(0,5,0.2), Reynolds_number(V=seq(0,5,0.2), D=0.002, nu=1), type="l", lty="dotted")
points(seq(0,5,0.2), Reynolds_number(V=seq(0,5,0.2), D=0.002, nu=1.5), type="l", lty="dotdash")
...
```{r, fig.height=4, fig.width=6}
plot(10:30, external_resistance_to_water_vapor_transfer(H=10:30), type="l", xlab = "heat transfer coefficient (W m^-2 C^-1)", ylab = "external resistance, r_e (s m^-1)")
```
...
legend("bottomright", title="Radiation, Sabs (W m^-2)", legend=c(400,600,800), lty=c("dashed","dotted","solid") )
```
...
legend("bottomright",title="direct radiation, H_sdif", legend=c(100,300,500), lty=c("dashed","solid","dotted") )
```
...
$$Q_c= h_T A_c (T_b-T_a). $$
We calculate $h_{cs}$ from the convective heat transfer coefficient as $h_{cs}=h_c(-0.007z/L+1.71)$ where $z$=0.001m is the height above the ground. We use an empirically-derived relationship for grasshoppers to estimate the heat transfer coefficient, $h_c (Wm^{-2}*^{\circ} C^{-1})$ [@lactin1998environmental]: $h_c = Nu K_f/L$, where the thermal conductivity of fluid, $K_f(W m^{-1}K^{-1})=0.024+0.00007(T_a+273)$.

We use an empirical relationship from Anderson et al. [-@lactin1998environmental] to estimate the Nusselt number, $Nu$, as $Nu=0.41 Re^{0.5}$ where $Re$ is the Reynolds number.  $Re=u L/v$, where $u$ is windspeed ($m/s$) and $v$ is the kinematic viscosity of air ($m^2/s$) ($v=15.68x10^{-6}$ at 300K).
...
legend("bottomright",title="direct radiation, H", legend=c(200,400,600), lty=c("dashed","dotted","solid") )
...
Suppose we want to estimate the hourly body temperature (operative environmental temperature, $T_e$) of a *Sceloporus* lizard on June 1, 2018 in Santa Fe, New Mexico, USA (35.69, -105.944, elevation: 2121m). Let's assume the lizard is in an unshaded location where the daily air temperature varies from a minimum of 10C to a maximum of 25C, the soil surface temperature varies from a minimum of 15C to a maximum of 30C, and the windspeed is 0.5 m/s. We assume that atmospheric transmissivity $\tau=0.7$ and albedo $\rho=0.6$. 
...
#Plot 
plot(1:24, Sdir, type="l", xlab = "hour", ylab = "radiation (W/m^2)", ylim=c(0,1200))
...
#Plot 
plot(1:24, Qnet, type="l", xlab = "hour", ylab = "heat flux (W/m^2)", ylim=c(-1,4))
