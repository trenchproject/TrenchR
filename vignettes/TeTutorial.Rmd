...
We will initially assume that $T_b=T_a+10$ to illustrate estimating components of the energy balance. We will then estimate $T_b$ based on the components of the energy balance.
...
Qemit= rep(NA, 24)
for(hr in 1:24) Qemit[hr]=Qemitted_thermal_radiation(epsilon=epsilon_s, A=A, psa_dir=psa[hr], psa_ref=1-psa[hr], T_b=Ta[hr]+10+273, T_g=Ts[hr]+273, T_a=Ta[hr]+273, enclosed=FALSE)
```
...
```{r}
Qconv= rep(NA, 24)
for(hr in 1:24) Qconv[hr]= Qconvection(T_a= Ta[hr]+273,T_b= Ta[hr]+10+273,H=H_L,A=A, proportion=0.67, ef=1.3)
```
...
```{r}
Qcond= rep(NA, 24)
for(hr in 1:24) Qcond[hr]=Qconduction_animal(T_g= Ts[hr]+273,T_b=Ta[hr]+10+273,d=10^-6,K=K,A=A, proportion=0.01)
```
...
#Plot 
plot(1:24, -Qnet, type="l", xlab = "hour", ylab = "heat flux (W/m^2)", ylim=c(0,10))
...
```{r}
TeGates= rep(NA, 24)
for(hr in 1:24) TeGates[hr]= Tb_Gates(A=A, D=svl, psa_dir=psa[hr], psa_ref=1-psa[hr], psa_air=0.67, psa_g=0.33, T_g=Ts[hr]+273, T_a=Ta[hr]+273, Qabs=Qabs[hr], epsilon=epsilon_s, H_L=H_L, ef=1.3, K=K)
```
