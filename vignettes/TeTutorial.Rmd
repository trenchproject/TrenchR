---
title: "TrenchR: Using energy balances to estimate body temperatures"
...
The body temperatures, $T_b$, of organisms can depart dramatically from air temperatures due to energy exchange with the environment.  Heat is gained by absorbing solar and thermal radiation and from metabolic reactions. Heat is lost through the organism's emission of radiation and the evaporation of water. The organism exchanges heat with the surrounding air or water via convection and with the ground and other solid surfaces in contact via conduction. The balance of these heat exchanges can be estimated and is referred to as operative environmental temperature, $T_e$. The $T_e$ indicates the steady-state temperature of an organism with given physical properties in a particular environment [@bakken1992measurement].

Calculations of operative environmental temperatures are based in balancing energy input and output to organisms.  The following accounting of heat gains and losses at the surface of the organism can be used to estimate $T_e$ [@gates2012biophysical]:
...
```
We briefly review each component of the energy balance below.  Details are available in biophysical ecology texts [@gates2012biophysical; @campbell2012introduction].

# Solar radiation, Qabs

The solar and thermal radiation absorbed by animals, $Q_{abs} (W)$, is the sum of direct $S_{dir}$, diffuse $S_{dif}$, and reflected $S_{ref}$ solar radiation ($W/m^2$). The sum is weighted by the organism's surface area $A (m)$ exposed to the radiation sources. Additionally, all forms of incoming radiation are multipled by the solar absorptivity of the animal surface ($a$ proportion) to estimate absorbed radiation. The sumation of incoming solar radiation is thus as follows:
...
Animals exchange heat with the air or water they are immersed in at a rate determined by the temperature difference between the temperature of the animal, $T_b (K)$, and that of the air or water, $T_a (K)$.  The rate is also determined by the animal's surface area exposed to the air or water, which we estimate from the input parameters of the animal's surface area, $A (m^2)$ and the proportion of the surface area in contact with the surrounding fluid, $proportion$. A heat transfer coefficient, $H_L (W m^{-2} K^{-1})$, which can be estimated using the relations below, is used to estimate the rate of heat exchange.  An enhancement factor multiplier, $ef$, can also be incorporated to account for increases in heat exchange resulting from air turbulance in field conditions. Conduction can be estimated as follows:
$$Qconv= ef\cdot H_L\cdot(A\cdot proportion)(Ta-Tb).$$
...
The Nusselt number provides a dimensionless estimate of conductance as is estimated as $Nu = H_L \cdot D / K$, where $H_L$ is the convective heat transfer coefficient ($W m^{-2} K^{-1}$), $D$ is the characteristic dimension ($m$), and $K$ is thermal conductivity ($W K^{-1}m^{-1}$). 
...
plot(seq(0,3,0.25), heat_transfer_coefficient_simple(V=seq(0,3,0.25),D=0.05), type="l", xlab = "Air velocity (m/s)", ylab = expression("heat transfer coefficient," ~ H[L] ~ (W ~ m^{-2} ~ K^{-1})))
points(seq(0,3,0.25), heat_transfer_coefficient_simple(V=seq(0,3,0.25),D=0.03), type="l", lty="dashed")
points(seq(0,3,0.25), heat_transfer_coefficient_simple(V=seq(0,3,0.25),D=0.07), type="l", lty="dotted")
...
$$Qcond= A\cdot proportion \cdot K(T_b-T_g)/d, $$
...
```{r, fig.height=4, fig.width=6}
plot(293:313, Tb_CampbellNorman(T_a=293:313, S=800, epsilon=0.96, c_p=29.3, D=0.17, V=1), type="l", xlab = "air temperature (K)", ylab = "body temperature (K)", xlim=c(295,310), ylim=c(290,325))
abline(a=0, b=1, col="gray")

points(293:313, Tb_CampbellNorman(T_a=293:313, S=400, epsilon=0.96, c_p=29.3, D=0.17, V=1), type="l", lty="dashed")
points(293:313, Tb_CampbellNorman(T_a=293:313, S=600, epsilon=0.96, c_p=29.3, D=0.17, V=1), type="l", lty="dotted")

legend("bottomright", title= expression("Radiation, Sabs" ~ (W ~ m^{-2})), legend=c(400,600,800), lty=c("dashed","dotted","solid") )
```
...
We adapted a biophysical model (`Tb_lizard()`) to calculate operative environmental temperature, $T_e (K)$, for lizards from Buckley [-@buckley2007linking]. The function follows the modeling approach contained Campbell and Norman [-@campbell2012introduction]. 
...
where $J$ is calendar day, with $J=1$ at January 1. One can then calculate day length, $h_{day} (h)$, using the CBM model [@forsythe1995model]: where $\phi$ is latitude (rad).
...
In addition, the offset needs to be taken into consideration. Although meridians are set every 15° and the time zones are supposed to equal to the time of the meridian that is closest to them, (thus anywhere between 7.5°E to 22.5°E should equal the time at the meridian at 15°E) the actual time zones does not necessarily follow the pattern. For, example, Madrid is located at 3.7°W but the time zone in Spain is set to that of 15°E (GMT+1). This produces an error in the calculations because noon at 3.7°W would be considered as equal to noon at 0° when in reality, it should be 11am of 0°. To eliminate this error, an offset value can be input.
...
Within the two values that are produced from this formula, the correct angle is determined based on the longtitude of the measured place.
...
$$Q_s= Q_t + Q_c,$$
where $Q_s$ is the total solar radiative heat flux ($W$), $Q_t$ is the thermal radiative heat flux ($W$), and $Q_c$ is the convective heat flux ($W$).  Conduction of heat between the body and vegetation and evaporative heat loss are considered to be negligible.  The solar radiative heat flux is
...
Each component is calculated as the product of the solar absorptivity of the grasshopper (we assume $\alpha=0.7$, [@anderson1979habitat]), the horizontal flux of solar radiation ($H_{s,dir}$, $H_{s,dif}$, and $H_{s,ttl}$ for the direct, diffuse, and total fluxes, respectively), and the silhouette area of the grasshopper exposed to solar radiation ($A_{s,dir}$, $A_{s,dif}$, and $A_{s,ttl}$ for the direct, diffuse, and total surface areas, respectively).  The direct radiation is adjusted for the zenith angle ($z$, degrees), which is the angle of the sun away from vertical.  

We calculate the surface area by approximating the body of a female grasshopper as a rotational ellipsoid [@samietz2005altitudinal].  The major axis is equal to the grasshopper’s length.  We calculate the semi-minor axis (half of the grasshopper’s width) as $a=(0.365+0.241\times1000 L)/1000 m$ using a regression from Lactin and Johnson [-@lactin1998environmental]. If $e=\sqrt{1-a^2/c^2}$, surface area can be calculated as follows:
...
We use an empirical relationship from Anderson et al. [-@lactin1998environmental] to estimate the Nusselt number, $Nu$, as $Nu=0.41 Re^{0.5}$ where $Re$ is the Reynolds number.  $Re=u L/v$, where $u$ is windspeed ($m/s$) and $v$ is the kinematic viscosity of air ($m^2/s$) ($v=15.68 \times 10^{-6}$ at $300K$).
...
We adapted a biophysical model for intertidal mussels which was developed by Helmuth [-@helmuth1998intertidal] to predict mussel body temperature $T_b (K)$ based on environmental conditions, body size (height and length) and whether the mussel is solitary or aggregated. The amount of heat stored within a mussel body $Q_{stored} (W)$ can be described as
...
Here, $\sigma$ is the Stefan-Boltzman constant ($5.66 \times 10^{-8}$), $T_a$ and $T_g$ are air and ground temperatures, respectively, and $A_{rad,sky}$ and $A_{rad,ground}$ are surface area subject to long-wave radiation from sky and ground respectively, both of which are simplified as half of the total surface area due to the nature of mussels. $\varepsilon_{org}$ and $\varepsilon_{surr}$ are the longwave thermal emissivities of the organism and its surroundings, which are roughly around 0.97. $\varepsilon_{sky}$ represents a functional infrared emissivity of the sky, which varies by air temperature and cloud cover.
...
Heat conduction $Q_{cond}$ can be calculated from $Q_{cond} = \frac{k_b}{0.5H}A_{cond}(T_b-T_g)$ where $k_b$ is the thermal conductivity of heat in mussel body. Because most of the body of a mussel is composed of water, we approximated it to be equal to that of water, $0.60 W m^{-2} K^{-1}$. $H$ represents the height of the mussel and $A_{cond}$ is the mussel area that is touching the ground.

In terms of convective heat flux ($Q_{conv}$), it can be calculated from $Q_{conv}=h_cA_{conv}(T_b-T_a)$ where $h_c$ is the coefficient for forced convection and $A_{conv}$ is total surface area exposed to convective heat loss [@helmuth1998intertidal]. We calculate the value of $h_c$ using three empirical formulas relating Nusselt numbers and Reynolds numbers: $Nu = \frac{h_c L}{K_a}$, $Re = \frac{uL}{v}$ and $Nu = aRe^b$, where $L$ is the length of the mussel, $K_a$ is the conductivity of air $K_a = 0.00501 + 7.2 \times10^{–5}T_a (W·m^{–1}·K^{–1})$, $u$ is the wind speed (m/s), and $v$ is the kinematic viscosity of air $v = -1.25 \times 10^{-5} + 9.2 \times 10^{-8} T_a$ [@denny2006hot]. $A_{conv}$ can be assumed equal to the total surface area. The authors empircally acquired $a$ and $b$ in the formula to obtain $Nu$, which vary depending on the position and the state of the mussels. For solitary mussels, $a = 0.38$ and $b = 0.51$ if the anterior or posterior end are facing upwind, and $a = 0.63$ and $b = 0.47$ if the valve is facing upwind. The values were measured to be $a = 0.67$, $b = 0.42$ for aggregated mussels [@helmuth1998intertidal].

Lastly, $Q_{evap} = \lambda \dot{m}$ where $\lambda$ is the latent heat of vaporization of water ($J/kg$) and $\dot{m}$ is the rate of water loss [@helmuth1998intertidal]. $\dot{m} = h_m \cdot A_{evap} \cdot (\rho_{body} - \rho_{air})$ where $h_m$ is a coefficient of mass transfer ($m/s$), $A_{evap}$ is the area of mass exchange ($m^2$) and $\rho$ is the vapor density ($kg m^{-3}$).

For simplicity, `Tb_mussel` function examines the steady-state (equilibrium) body temperature of an organism exposed over long periods of time to constant environmental conditions.
...
t.seq <- lapply(20:40, FUN = Tb_mussel, L = 0.1, H = 0.05, T_g = 30, u = 0.2, p = 0.03, psi = 30, elev = 500, evap = 0, rho_diff = 0.0001, cl = 0, group = "solitary")

plot(20:40, t.seq, type = "l", xlab = "ambient temperature (°C)", ylab = "body temperature (°C)", ylim = c(20, 40))
t.seq <- lapply(20:40, FUN = Tb_mussel, L = 0.1, H = 0.05, T_g = 30, u = 1, p = 0.03, psi = 30, elev = 500, evap = 0, rho_diff = 0.0001, cl = 0, group = "solitary")
points(20:40, t.seq, type = "l", lty = "dashed")

t.seq <- lapply(20:40, FUN = Tb_mussel, L = 0.1, H = 0.05, T_g = 30, u = 10, p = 0.03, psi = 30, elev = 500, evap = 0, rho_diff = 0.0001, cl = 0, group = "solitary")
...
We adapted a biophysical model `Tb_Limpet()` from Denny and Harley [-@denny2006hot] to predict the body temperature of limpets based on body size (length and height), environmental conditions and the position of the limpet. In this model, the shape of the limpet is approximated to a cone of radius $r$ and height $H$.

The rate at which energy is stored within the organisms ($W_{stored} (W)$) are influenced by several heat exchanging processes. The processes include the rate of heat absorbed from short-wave radiation ($W_{sw} (W)$), net heat transferred by long-wave radiation ($W_{lw} (W)$), rate of heat transferred by conduction and convection ($W_{cd} (W)$ and $W_{cv} (W)$). We neglected the heat gain by metabolism due to the nature of the limpets' low metabolic rate, and evaporative heat loss assuming that the limpet has its shell firmly clamped against the substratum, and thereby preventing evaporation.
...
where $\psi$ is the solar zenith angle [@pennell1989computing]. $\alpha_{sw}$ was emperically acquired to be 0.68 for limpets [@denny2006hot].
...
In terms of convective heat flux ($Q_{cv}$), 
$$W_{cv} = h_cA_{cv}(T_a-T_b)$$
which is derived from the Newton's law of cooling.
$A_{cv}$ is the area of the shell in contact with the air ($m^2$), which is approximated to the lateral area of the limpet ($A_l$). $h_c$ is the convective heat transfer coefficient ($W m^{-2} K^{-1}$), and it can be expressed from Nusselt number $Nu = \frac{h_c R}{K_a}$ where $R$ is the diameter and $K_a$ is the conductivity of air ($W m^{-1} K^{-1}$).
$K_a = 0.00501 + 7.2 \times 10^{-5}T_a$ is known from the study done by Denny (1993).
Additionally, 
$$Re = \frac{uR}{v}$$
and
$$Nu = aRe^b$$
where $u$ is the wind speed ($m/s$), $v$ is the kinematic viscosity of air ($m^2/s$) that equals $-1.25 \times 10^{-5} + 9.2 \times 10^{-8} T_a$, and $a$ and $b$ are coefficients that need to be emperically obtained. 
When the anterior side of the limpet is facing upwind, $a=1.955$ and $b=0.371$. When the posterior side is facing upwind, $a=1.881$ and $b=0.376$. Lastly when the broadside is facing upwind, $a=1.304$ and $b=0.404$ [@denny2006hot]. 

Conductive heat exchange is determined by the following equation.
$$W_{cd} = A_{cd}K_r(\frac{dT_r}{dz})$$
...
plot(sqrt(5:20), blr.seq, type = "l", xlab = "diameter (m)", ylab = expression("boundary layer resistance" ~ (s ~ cm^{-1})))
...
psa= sapply(psi_deg, prop_silhouette_area, taxa= "lizard", posture= "elevated")
...
H_L=heat_transfer_coefficient(V=V,D=svl,K=K, nu= 15.3 * 10^(-6) , taxa="lizard_surface")

#Also illustrate estimations using a spherical approximation and a simplified version of the approximation.
heat_transfer_coefficient_approximation(V=V,D=svl,K= K, nu= 15.3 * 10^(-6), taxa="lizard")
...
```{r}
TeCN= rep(NA, 24)
for(hr in 1:24) TeCN[hr]= Tb_CampbellNorman(T_a=Ta[hr]+273, S=Qabs[hr], epsilon=epsilon_s, c_p=29.3, D=svl, V=V)
```
...
TeFei= rep(NA, 24)
for(hr in 1:24) TeFei[hr]= Tb_Fei(T_a=Ta[hr]+273, T_g=Ts[hr]+273, H=Qabs[hr], lw=30, shade=0.5, m=10.5, Acondfact=0.05, Agradfact=0.4)
```
...
plot(1:24, TeGates-273, type="l", xlab="Hour", ylab="Temperature (°C)", col="blue", ylim=c(0,80)) #Gates
...
We will now use the body temperature estimates to examine the components of the energy balance. We will be solving the following energy balance:
...
We estimate thermal radiation $Q_{emit}$ for the lizard outdoors. We assume the surface emisivity of lizards, $\epsilon_s=0.965$ [@barlett1967energy].
...
$$Q_{conv}= ef\cdot H_L(A*proportion)(Ta-Tb),$$
