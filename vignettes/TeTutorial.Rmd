...
Heat conduction $Q_{cond}$ can be calculated from $Q_{cond} = \frac{k_b}{0.5H}A_{cond}(T_b-T_g)$ where $k_b$ is the thermal conductivity of heat in mussel body. Because most of the body of a mussel is composed of water, we approximated it to be equal to that of water, $0.60 W m^{-2} K^{-1}$ (Denny 1993). $H$ represents the height of the mussel and $A_{cond}$ is the mussel area that is touching the ground.

In terms of convective heat flux ($Q_{conv}$), it can be calculated from $Q_{conv}=h_cA_{conv}(T_b-T_a)$ where $h_c$ is the coefficient for forced convection and $A_{conv}$ is total surface area exposed to convective heat loss (Helmuth 1998). Using the three empirical formulas on Nusselt number and Reynolds number $Nu = \frac{h_c L}{K_a}$, $Re = \frac{uL}{v}$ and $Nu = aRe^b$, the value of $h_c$ can be calculated. $L$ is the length of the mussel, $K_a$ is the conductivity of air $K_a = 0.00501 + 7.2 \times10^{–5}T_a (W·m^{–1}·K^{–1})$, $u$ is the wind speed (m/s), $v$ is the kinematic viscosity of air $v = -1.25 \times 10^{-5} + 9.2 \times 10^{-8} T_a$ (Denny and Harley 2006). $A_{conv}$ can be considered to be equal to the total surface area. $a$ and $b$ in the formula to obtain $Nu$ vary depending on the position and the state of the mussels, and they are empirically acquired. For solitary mussels, $a = 0.38$ and $b = 0.51$ if the anterior or posterior end are facing upwind, and $a = 0.63$ and $b = 0.47$ if the valve is facing upwind. The values were measured to be $a = 0.67$, $b = 0.42$ for aggregated mussels (Helmuth 1998).
...
We provide the operative temperature model as follows
<!-- {r, fig.height=4, fig.width=6} -->
<!-- t.seq <- lapply(20:40, FUN = Tb_limpet, L = 0.0176, H = 0.0122, T_r = 30, I = 500, u = 1, psi = 30, c = 0, position = "anterior") -->
<!-- plot(20:40, t.seq, type = "l", xlab = "ambient temperature (°C)", ylab = "body temperature (°C)", ylim = c(25, 45)) -->
<!-- t.seq <- lapply(20:40, FUN = Tb_limpet, L = 0.0176, H = 0.0122, T_r = 30, I = 800, u = 1, psi = 30, c = 0, position = "anterior") -->
<!-- points(20:40, t.seq, type = "l", lty = "dashed") -->
<!-- t.seq <- lapply(20:40, FUN = Tb_limpet, L = 0.0176, H = 0.0122, T_r = 30, I = 1200, u = 1, psi = 30, c = 0, position = "anterior") -->
<!-- points(20:40, t.seq, type = "l", lty = "dotted") -->
