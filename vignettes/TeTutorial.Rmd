...
where $A$ is surface area ($m^2$), $r_e$ is the external (convective) resistance to water vapor transport ($s/m$), $\rho_s$ is the saturation water vapor density at skin surface ($kg/m^3$), $h$ is relative humidity (between 0 ~ 1), and $\rho_a$ is the saturation water vapor density in ambient air ($kg/m^3$).
...
We adapted a biophysical model for intertidal mussels which was developed by Helmuth (1998) to predict mussel body temperature ($T_b$) based on environmental conditions, body size (height and length) and whether the mussel is solitary or aggregated.
...
where $Q_{sol}$ is the short-wave solar radiation to the mussel, $Q_{rad,sky}$, $Q_{rad,ground}$ are the infrared radiation between the mussel and the sky, and between the mussel and the surroundings, $Q_{conv}$ is the convective heat exchange, $Q_{cond}$ is the condunction to and from the ground, and $Q_{evap}$ is the evaporative cooling from water loss (Helmuth 1998).
...
We calculate $Q_{sol}$ from $Q_{sol} = \frac{\alpha}{sin\theta} A_{sol}S$ where $\alpha$ is the solar absorptivity, $\theta$ is the solar elevation angle (radian), $A_{sol}$ is the mussel area in direction of the sun, $S$ is the direct solar flux density on the area (Helmuth 1998). 

$Q_{rad,sky}$ and $Q_{rad,ground}$ can be calculated as follows (Helmuth 1998).
...
Here, $\sigma$ is the Stefan-Boltzman constant ($5.66 \times 10^{-8}$), $T_a$ and $T_g$ are air and ground temperatures, respectively, and $A_{rad,sky}$ and $A_{rad,ground}$ are surface area subject to long-wave radiation from sky and ground respectively, both of which are simplfied as half of the total surface area due to the nature of mussels. $\varepsilon_{org}$ and $\varepsilon_{surr}$ are the longwave thermal emissivities of the organism and its surroundings, which are roughly around 0.97. $\varepsilon_{sky}$ represents a functional infrared emissivity of the sky, which varies by air temperature and cloud cover.
In Environmental Biophysics (Campbell and Normal 1998), $\varepsilon_{sky}$ is given by $\varepsilon_{sky}=(1 - 0.84 \times c)(9.2 \times 10^{-6} \times T_a^2) + 0.84 \times c$ where $c$ represents the fraction of the sky covered by cloud.
...
In terms of convective heat flux ($Q_{conv}$), it can be calculated from $Q_{conv}=h_cA_{conv}(T_b-T_a)$ where $h_c$ is the coefficient for forced convection and $A_{conv}$ is total surface area exposed to convective heat loss (Helmuth 1998). Using the three emperical formala on Nusselt number and Reynolds number $Nu = \frac{h_c L}{K_a}$, $Re = \frac{uL}{v}$ and $Nu = aRe^b$, the value of $h_c$ can be calculated. $L$ is the length of the mussel, $K_a$ is the conductivity of air $K_a = 0.00501 + 7.2 \times10^{–5}T_a (W·m^{–1}·K^{–1})$, $u$ is the wind speed (m/s), $v$ is the kinematic viscosity of air $v = -1.25 \times 10^{-5} + 9.2 \times 10^{-8} T_a$ (Denny and Harly 2006). $A_{conv}$ can be considered to be equal to the total surface area. $a$ and $b$ in the formula to obtain $Nu$ vary depending on the position and the state of the mussels, and they are emperically acquired. For solitray mussels, $a = 0.38$ and $b = 0.51$ if the anterior or posterior end are facing upwind, and $a = 0.63$ and $b = 0.47$ if the valve is facing upwind. The values were measured to be $a = 0.67$, $b = 0.42$ for aggregated mussels (Helmuth 1998).
