...
Suppose we want to estimate the hourly body temperature (operative environmental temperature, $T_e$) of a Sceloporus lizard on June 1, 2018 in Santa Fe, New Mexico, USA (35.69, -105.944, elevation:2121). Let's assume the lizard is in an unshaded location where the daily air temperature varies from a minimum of 10C to a maximum of 25C, the soil surface temperature varies from a minimum of 15C to a maximum of 30C, and the windspeed is 0.5 m/s.
...
##Environmental data
Let us first prepare the environmental data for analysis. We will need to estimate hourly air and soil temperatures and radiation. We start be estimating day of year and the timing of sunrise and sunset:
```{r}
...
```{r}
Ta= sapply(1:24, diurnal_temp_variation_sineexp, Tmx=Tmax, Tmn=Tmin, tr=tr, ts=ts, alpha=2.59, beta= 1.55, gamma=2.2)
Ts= sapply(1:24, diurnal_temp_variation_sine, Tmx=Tmax_s, Tmn=Tmin_s)
```
...
Qemit= rep(NA, 24)
for(hr in 1:24) Qemit[hr]=Qemitted_thermal_radiation(epsilon=epsilon_s, A=A, psa_dir=psa[hr], psa_ref=1-psa[hr], Tb=Ta[hr], Tg=Ts[hr], Ta=Ta[hr], enclosed=FALSE)
```

We next estimate convection $Q_{conv}$:
```{r}
Qconvection(Ta= 293,Tb= 303,H_L=10.45,A=0.0025, proportion=0.85)
```

We will estimate the heat transfer coefficient using an empirical function for lizards, but also illustrate a generic function for estimating $H_L$.  

[the characteristic dimension $D (m)$, the thermal conductivity of the air $K (W m^{-1} K^{-1})$, the kinematic viscocity of the air $nu (m^2 s^{-1})$, and the taxa or a generic shape.]
```{r}
H_L=heat_transfer_coefficient_lizard(V=V, orientation="parallel")
#### FIX: H_L_gen= heat_transfer_coefficient(V=3,D=0.05,K= 25.7 * 10^(-3), nu= 15.3 * 10^(-6), "cylinder")
```

We estimate conductance assuming conductancer through the animal tissue is the rate limiting step.
...
We assume, as in generally done for lizards, that heat exchange associate with metabolism and evaporation is neglidglible:
```{r}
Qmet= 0
Qevap= 0
```
