...
$$Q_t= 0.5A_t \epsilon \sigma ((T_b+273.15)^4-(T_{sky}+273.15)^4)+0.5A_t \epsilon \sigma ((T_b+273.15)^4-(T_g+273.15)^4),$$

where $A_t$ is the thermal radiative heat transfer surface area ($m^2$), $T_b$ is the body temperature ($C$), $T_g$ is the ground surface temperature ($C$), $T_{sky}$ is the equivalent black body sky temperature ($C$), $\epsilon$ is butterfly thermal emissivity, and $\sigma$ is the Stefan-Boltzmann constant ($5.67\times10^{-8} W m^{-2} C^{-4}$).

The convective heat flux is given by: 

$$Q_c= h_T A_c (T_b-T_a), $$ 

where $A_c$ is the convective heat transfer surface area and $T_a$ is the air temperature ($C$). We assume $A_c=A_t= A_{s,ttl}$. The total convective heat transfer coefficient, $h_T Wm^{-2}C^{-1}$, is calculated as the boundary layer conductance $h_c Wm^{-2}C^{-1}$ and the fur layer conduction in series:

$$1/h_T= 1/h_c+\frac{(r_i+\delta)ln[(r_i+\delta)/r_i]}{k_e} ,$$ 

where $\delta$ is the thoracic fur thickness ($m$) and $k_e$ is the thermal conductivity of the fur ($Wm^{-1}C^{-1}$).
  
The boundary layer conductance, $h_c$, can be estimated using the relationship between two non-dimensional numbers. The Nusselt number, $nu =h_c D/ka$, is the ratio of convective to conductive heat transfer, where $k_a$ is the thermal conductivity of air ($Wm^{-1}C^{-1}$). We used the maximum width of the mesothorax as the characteristic dimension of the butterfly, $D (m)$. The Reynolds number, $Re =uD/v$, is the ratio of inertial forces to viscous forces, where $u$ is wind speed ($m/s$) and $v$ is kinematic viscosity ($m^2/s$). We used the Nu-Re relation for a cylinder, $nu = 0.6Re^{0.5}$, which is a reasonable approximation for *Colias*. We estimate the energy flux balance to estimate $T_b$.
...
We estimate thermal radiative flux as the sum of radiation from the sky and ground. We assume that one half of the grasshopper's body is subject to atmospheric radiation and the other half is subject to thermal radiation from the ground surface. Thermal radiation is calculated using the Stefan-Boltzmann law, which states that radiative flux is proportional to the fourth power of the absolute temperature of a body. Here $T_b$ is the body temperature (°C), $T_g$ is the ground surface temperature (°C), and $T_{sky}$ is the black body sky temperature (C) that corresponds to the air temperature [$T_{sky}= 0.0552(T_a+273.15)^{1.5}$, [@swinbank1963long]]. The Stefan-Boltzmann constant ($\sigma$) characterizes the proportionality of this relationship. The thermal emissivity ($\epsilon$) accounts for incomplete absorption or emission of thermal radiation, but in this case, we assume that both the grasshopper and ground are perfect black bodies ($\epsilon= 1$). We account for the thermal radiative heat transfer surface area ($A_t=A$). The relationship is thus:

$$Q_t= 0.5A_t \epsilon \sigma ((T_b+273.15)^4-T_{sky}^4)+0.5A_t \epsilon \sigma ((T_b+273.15)^4-(T_g+273.15)^4).$$

Convective heat flux is estimated as the product of the convective heat transfer coefficient in turbulent air $h_{cs} (Wm^{-2}C^{-1})$, the grasshopper's surface area exposed to convective heat flux ($A_c=A$), and the temperature difference between the grasshopper's body temperature $T_b$ and air temperature $T_a$:
