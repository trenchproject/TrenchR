...
```{r}
Qconv= rep(NA, 24)
for(hr in 1:24) Qconv[hr]= Qconvection(Ta= Ta[hr],Tb= Ta[hr],H_L=H_L,A=A, proportion=0.67, ef=1.3)
```
...
```{r}
Qcond= rep(NA, 24)
for(hr in 1:24) Qcond[hr]=Qconduction_animal(Tg= Ts[hr],Tb=Ta[hr],d=10^-6,K=K,A=A, proportion=0.33)
```
...
```{r}
Te= rep(NA, 24)
for(hr in 1:24) Te[hr]= Tb_Gates(A=A, l=svl, psa_dir=psa[hr], psa_ref=1-psa[hr], psa_air=0.67, psa_g=0.33, Tg=Ts[hr]+273, Ta=Ta[hr]+273, Qabs=Qabs[hr], epsilon=epsilon_s, H_L=H_L, ef=1.3, K=K)
```

We also implement a similiar but simplied energy balance:
```{r}
Te2= rep(NA, 24)
for(hr in 1:24) Te2[hr]= Tb_CampbellNorman(Ta=Ta[hr]+273, Sabs=Qabs[hr]/svl, epsilon=epsilon_s, c_p=29.3, D=svl, V=V)
#CHECK
```
...
```{r}
Te3= rep(NA, 24)
for(hr in 1:24) Te3[hr]= Tb_lizard(Ta=Ta[hr], Tg=Ts[hr], u=V, svl=svl*1000, mass=mass, psi=psi_deg[hr], rho_S=rho, elevation=elev, doy=doy, sun=TRUE, surface=TRUE, alpha_S=a, alpha_L=0.965, epsilon_s=epsilon_s, F_d=0.8, F_r=0.5, F_a=0.5, F_g=0.5)
```
...
```{r}
plot(1:24, Te-273, type="l", xlab="Hour", ylab="Temperature (C)", col="blue")
points(1:24, Te2-273, type="l", col="blue", lty="dotted")
points(1:24, Te3, type="l", col="blue", lty="dashed")
...
legend("topleft",
       ...
       lty=c(1,0), pch=NA, col=c("orange", "purple", "blue","green"))
